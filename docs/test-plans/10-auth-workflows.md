...
| Cedar shadow authorization schema and compatibility parity | Automated | `cargo test -p layerhouse-server auth::cedar_shadow` | P1 | Implemented as production-compiled, non-enforcing shadow telemetry; compatibility authorizer remains authoritative | command log |
