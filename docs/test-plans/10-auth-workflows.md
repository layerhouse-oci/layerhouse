...
| Cedar shadow authorization schema and compatibility parity | Automated | `cargo test -p layerhouse-server auth::cedar_shadow` | P1 | Implemented as test-only coverage; production enforcement remains on the compatibility authorizer | command log |
