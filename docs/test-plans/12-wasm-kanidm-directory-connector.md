...
|---|---|---|---|---|---|
| WIT ABI round trip with `crates/connectors/fake-directory` | Automated | `cargo test -p layerhouse-server directory_wasm` | P0 | Planned; not implemented yet | command log |
| Config/startup matrix for disabled directory, enabled missing component, digest mismatch, ABI mismatch, and token/base-origin validation | Automated | `cargo test -p layerhouse-server directory_config` | P0 | Planned; not implemented yet | command log |
